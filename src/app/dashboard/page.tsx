  ...
  return (
    <div className="max-w-5xl">
      {welcome === "1" && (
        <div className="mb-8 rounded-sm border border-success/30 bg-success/5 px-5 py-4">
          <p className="text-sm text-off-white">
            ...
            for a confirmation email, then connect a wallet below to claim
            your reward.
          </p>
        </div>
      )}
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-3">
        <StatCard label="Token balance" value={user.tokenBalance} suffix={TOKEN_SYMBOL} />
        <StatCard label="GPU sessions" value={gpuCount} />
        <StatCard label="Skills owned" value={skillCount} />
      </div>

      <div className="mt-8 grid gap-6 lg:grid-cols-[1.3fr_1fr]">
        <WalletConnectCard initialUser={user} />
        ...
        <div className="overflow-hidden rounded-sm border border-border">
          {transactions.length === 0 ? (
            <p className="bg-panel px-5 py-8 text-center text-sm text-text-muted">
              No activity yet — connect a wallet to get started.
            </p>
          ) : (
